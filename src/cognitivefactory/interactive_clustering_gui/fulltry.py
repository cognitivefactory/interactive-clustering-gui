...
class accueil(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Button(self, text="Importer", command=lambda: [self.import_nom(controller, l)]).pack(
            side=tk.LEFT, padx=5, pady=5
        )
        tk.Button(self, text="Charger", command=lambda: self.import_session(controller, l)).pack(
...
class accueil_import_nom(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Button(
            self,
            text="Valider",
            command=lambda: [self.get_name(entree.get()), print(SESSIONNAME), controller.show_frame("commencer")],
